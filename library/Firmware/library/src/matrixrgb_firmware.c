...
*******************************************************************************/
#define Clk_Toggle()                Led_Matrix_Clk = 0; Delay_us(1); Led_Matrix_Clk = 1; Delay_us(1); Led_Matrix_Clk = 0;
#define Latch_Off()                 Led_Matrix_STB = 0;  asm nop; asm nop; asm nop;
#define Latch_On()                  Delay_us(1); Led_Matrix_STB = 1;
...
uint8_t currRow      = 0;           /**< Current row of display */
uint8_t p_width, p_height;          /**< Width and Height in pixels */
...
void write_text( uint8_t *text, color_t color, uint8_t start_row, uint8_t start_col )
{
      ...
      uint8_t curr_row = start_row;    //Current row and column are top left of letter
...
void display_shift_left( void )
{
     ...
     for( row = 0; row < p_height; row++ )
     {
         for( column = 0; column <= ( p_width - 1 ); column++ )
         {
             ...
             temp_col+= ( temp_col / 32 ) * 32;  //Used because panels come connected in 32 x 32 packages ... SOMETIMES :(
...
void write_pixel( int row, int column, char red, char green, char blue)
{
    ...
    if( blue > 0 )
        buffer |= 0x01;
    column+= ( column / 32 ) * 32;  //Used because panels come connected in 32 x 32 packages ... SOMETIMES :(
    FB[ incrementer + ( starter *  shift_reg ) + column] |= buffer;
